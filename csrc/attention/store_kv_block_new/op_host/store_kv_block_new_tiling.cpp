...
#include "store_kv_block_new_tiling.h"
...
static ge::graphStatus DoCommonTiling(gert::TilingContext* context, StoreKVBlockNewParams& params) {
    auto kShape = context->GetInputShape(DIM_0);
    auto kDimNum = kShape->GetStorageShape().GetDimNum();
    if (kDimNum < 2 || kDimNum > 7) {
        OP_LOGE(context->GetNodeName(), "StoreKVBlockNew Input kDimNum dim < 2 || kDimNum>7");
    ...
    if (kCacheDimNum < 2 || kCacheDimNum > 7) {
        OP_LOGE(context->GetNodeName(), "StoreKVBlockNew Input kCacheDimNum < 2");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus StoreKVBlockNewTilingFunc(gert::TilingContext* context) {
    StoreKVBlockNewParams params;
    ...
    // Query the actual UB size and derive the max number of tokens that can be
    // transferred in a single UB buffer, so that an oversized group is split
    // into multiple copies in the kernel instead of failing.
    uint64_t ubSize = 0;
    ascendcPlatform.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
    uint32_t usableUbSize = (ubSize < MAX_UB_USE_SIZE) ? static_cast<uint32_t>(ubSize) : MAX_UB_USE_SIZE;
    uint32_t tokenByteSize = params.tokenSize * params.typeByte;
    if (tokenByteSize == 0 || usableUbSize < tokenByteSize) {
        OP_LOGE(context->GetNodeName(), "token byte size too large for UB.");
        return ge::GRAPH_FAILED;
    }
    params.maxTokensPerCopy = usableUbSize / tokenByteSize;

    StoreKVBlockNewTilingData tilingData;
    ...
    tilingData.set_maxTokensPerCopy(params.maxTokensPerCopy);
...
static ge::graphStatus TilingParseForStoreKVBlockNew(gert::TilingParseContext* context) {
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(StoreKVBlockNew)
    .Tiling(StoreKVBlockNewTilingFunc)
    .TilingParse<StoreKVBlockNewCompileInfo>(TilingParseForStoreKVBlockNew);
